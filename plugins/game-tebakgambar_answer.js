...
export async function before(m) {
    ...
    if (!(id in this.tebakgambar))
        return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, buttonTebakgambar, m)
    if (m.quoted.id == this.tebakgambar[id][0].id) {
        let isSurrender = /^((me)?nyerah|surr?ender)$/i.test(m.text)
        if (isSurrender) {
            clearTimeout(this.tebakgambar[id][3])
            delete this.tebakgambar[id]
            return conn.sendButton(m.chat, '*Yah Menyerah :( !*', wm, null, buttonTebakgambar, m)
        }
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.tebakgambar[id][2]} XP`, wm, null, buttonTebakgambar, m)
            clearTimeout(this.tebakgambar[id][3])
            delete this.tebakgambar[id]
