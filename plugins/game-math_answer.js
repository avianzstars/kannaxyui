...
handler.before = async function (m) {
    ...
    if (!(id in this.math)) return conn.sendButton(m.chat, 'Soal itu telah berakhir', wm, null, [['math', '/math']], m)
    if (m.quoted.id == this.math[id][0].id) {
        ...
        if (m.text == math.result) {
            ...
            delete this.math[id]
            conn.sendButton(m.chat, `*Jawaban Benar!*\n+${math.bonus} XP`, wm, null, [['again', `/math ${math.mode}`]], m)
        } else {
            if (--this.math[id][2] == 0) {
                clearTimeout(this.math[id][3])
                delete this.math[id]
                conn.sendButton(m.chat, `*Kesempatan habis!*\nJawaban: *${math.result}*`, wm, null, [['again', `/math ${math.mode}`]], m)
